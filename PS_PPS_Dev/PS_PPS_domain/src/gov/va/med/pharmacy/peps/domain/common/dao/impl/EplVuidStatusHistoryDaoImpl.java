/**
 * Source file created in 2007 by Southwest Research Institute
 */


package gov.va.med.pharmacy.peps.domain.common.dao.impl;


import gov.va.med.pharmacy.peps.domain.common.dao.EplVuidStatusHistoryDao;
import gov.va.med.pharmacy.peps.domain.common.model.EplVuidStatusHistoryDo;


/**
 * This class makes avaliable the methods that may be used to access and 
 * modify the data referenced by the EplOrderUnitDao.
 */
public class EplVuidStatusHistoryDaoImpl extends DataAccessObjectImpl<EplVuidStatusHistoryDo, Long> 
    implements EplVuidStatusHistoryDao {

}
