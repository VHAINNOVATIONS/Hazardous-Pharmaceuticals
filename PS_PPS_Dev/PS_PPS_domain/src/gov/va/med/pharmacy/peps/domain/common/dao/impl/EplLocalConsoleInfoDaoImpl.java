/**
 * Source file created in 2006 by Southwest Research Institute
 */


package gov.va.med.pharmacy.peps.domain.common.dao.impl;


import gov.va.med.pharmacy.peps.domain.common.dao.EplLocalConsoleInfoDao;
import gov.va.med.pharmacy.peps.domain.common.model.EplLocalConsoleInfoDo;


/**
 * This class makes avaliable the methods that may be used to access and 
 * modify the data referenced by the EplLocalConsoleInfoDao.
 */
public class EplLocalConsoleInfoDaoImpl 
    extends DataAccessObjectImpl<EplLocalConsoleInfoDo, String> 
    implements EplLocalConsoleInfoDao {

}
