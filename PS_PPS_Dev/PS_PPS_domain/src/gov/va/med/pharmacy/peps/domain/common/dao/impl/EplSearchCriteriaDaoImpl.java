/**
 * Source file created in 2008 by Southwest Research Institute
 */


package gov.va.med.pharmacy.peps.domain.common.dao.impl;


import gov.va.med.pharmacy.peps.domain.common.dao.EplSearchCriteriaDao;
import gov.va.med.pharmacy.peps.domain.common.model.EplSearchCriteriaDo;
import gov.va.med.pharmacy.peps.domain.common.model.EplSearchCriteriaDoKey;


/**
 * This class makes avaliable the methods that may be used to access and 
 * modify the data referenced by the EplSearchCriteriaDao.
 */
public class EplSearchCriteriaDaoImpl 
    extends DataAccessObjectImpl<EplSearchCriteriaDo, EplSearchCriteriaDoKey> 
    implements EplSearchCriteriaDao {

}
