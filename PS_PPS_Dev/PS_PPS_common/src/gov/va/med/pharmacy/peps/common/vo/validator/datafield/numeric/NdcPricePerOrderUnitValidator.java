...
/**
 * Validate NDC price per order unit VA data field
 */
public class NdcPricePerOrderUnitValidator extends AbstractValidator<DataField<Double>> {
    ...
    public void validate(DataField<Double> target, Errors errors) {
        ...
        if (isNull(target)) {
            rejectIfNull(target, errors, FieldKey.NDC_PRICE_PER_ORDER_UNIT);

            return;
        }

        if (isNull(target.getValue())) {
            rejectIfNull(target.getValue(), errors, FieldKey.NDC_PRICE_PER_ORDER_UNIT);

            return;
        }
        
        rejectIfOutsideRangeInclusive(target.getValue(), 0, PPSConstants.I999999, errors, 
            FieldKey.NDC_PRICE_PER_ORDER_UNIT);
        ...
        if (!matchFound1) {
            errors.addError(ErrorKey.COMMON_INCORRECT_DECIMAL_DIGITS, new Object[] {FieldKey.NDC_PRICE_PER_ORDER_UNIT, 2});
        }
