...
function TfrmFiveRights.ProcessViaFiveRightsLogic(MedOrder: TBCMA_MedOrder):
  boolean;
const
  CRLF = #13#10;
var
...
begin
  with MedOrder do begin
    if OrderTypeID = otUnitDose then begin
      ...
      for i := 0 to MedOrder.Additives.Count - 1 do begin
        AdditiveList := AdditiveList + '  ' + piece(MedOrder.Additives[i], '^',
          3) + ' ' +
          piece(MedOrder.Additives[i], '^', 4) + CRLF;
        ScanDrug(piece(MedOrder.Additives[i], '^', 1), True);
      end;

      SolutionList := 'Solutions:' + CRLF;

      for i := 0 to MedOrder.Solutions.Count - 1 do begin
        SolutionList := SolutionList + '  ' + piece(MedOrder.Solutions[i], '^',
          3) +
          piece(MedOrder.Solutions[i], '^', 4) + CRLF;
        ScanDrug(piece(MedOrder.Solutions[i], '^', 1), True);
      end;
      ...
      if DefMessageDlg(DisplayList, mtConfirmation, [mbYes, mbCancel], 0) = mrYes
        ...
        frmMain.CreateWardStockForFiveRightsOverride(MedOrder, WardStockBagName,
        ...
        if UAS_LogState = LA_OkToLog then begin
          ...
          //                            'M');
                      ...
                      {JK 8/14/2008}
          if MedOrder.Route = 'IV PIGGYBACK' then
            RouteText := 'IVPB'
          else
            RouteText := 'IV';
          ...
          try
            if IVHistoryDates.Count > 0 then
              LogUnableToScan(MedOrder.OrderNumber, UASReason,
                BCMA_Common.BuildCommentLine('[' + RouteText + ' Bag ' +
                TBCMA_IVBAGS(IVHistoryDates.Items[0]).UniqueID +
                ' built during UTS - Verify 5 Rights Override]', MemComment),
                UnableToScanString, 'M')
