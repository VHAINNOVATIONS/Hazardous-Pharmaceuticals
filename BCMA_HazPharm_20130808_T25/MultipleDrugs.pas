...
//      if not DoseFound then begin  //bjr 1/11/2012, BCMA00000944
    ...
    if (not DoseFound) and (MappedIEN <> '') then begin  // bjr 4/16/2012, BCMA00000944
      DefMessageDlg('Invalid medication lookup.' + #13#10#13#10 + 'DO NOT GIVE!', mtError, [mbOK], 0); //bjr 1/11/2012, BCMA00000944
    end;
end;
...
//                      '', 40, True, False, CheckState, '');
    ...
    if Result = -1 then
      MessageDlg('Scanned Drug has already been given or,' + #13 +
...
//            CheckState, '');
        ...
        begin
          ...
          DefMessageDlg('Quantity and Units are required and cannot be null or a space. '
            +
            'The scanned medication has been canceled.', mtWarning, [mbOK], 0);
          Exit;
        end;
      end;
  end;
...
//        DrugScanned := True;
  ...
  begin
    DefMessageDlg('No medications have been scanned or the scanned medication has been canceled!', mtInformation, [mbOK], 0);
    if edtScannerInput.Enabled then
      edtScannerInput.setFocus;
    CloseFrm := False;
  end
