...
{$R *.DFM}
uses
  BCMA_Startup, BCMA_Main, BCMA_Common, MFunStr, Math;
...
begin
  ...
  // if only one word, reject input
//  if (Pos(' ', tmpstr) = 0) and (Pos(',', tmpstr) = 0) then
//    Result := False

  // The rest of these tests are expecting one-word input which has already been
  // rejected.  Will only trap integer, IV label, and drug code errors when
...
//    TestSet := [' ', '0'..'9','U'];
  ...
  if Result then begin
    ...
    if msgstr = '' then
//      msgstr := 'Error!  Scanned input not allowed in this dialog.  ' +
//      'Please type the Quantity and Units associated with this administration.';
