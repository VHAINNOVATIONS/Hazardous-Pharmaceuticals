...
//  ErrIncompleteData = 'Incomplete data returned from VistA';
  ...
  ErrIncompleteDataUtil = 'Incomplete data returned from System'; // rpk 7/13/2010
...
// remove line breaks (CR-LF) from string
...
begin {FormatFMDateTime}
  ...
  while Length(AFormat) > 0 do
    case UpCase(AFormat[1]) of
      '"': begin // literal
          Delete(AFormat, 1, 1);
          while not (CharAt(AFormat, 1) in [#0, '"']) do begin
      ...
      'M': case TrimFormatCount of // month
          ...
          //the pharmacy package
          3: {//if m in [1..12] then Result := Result + MONTH_NAMES_SHORT[m];} begin
              if m in [1..12] then
...
begin
  Result := ADateTime;
  if IsFMDateTime(ADateTime) then begin
...
begin
  Result := False;
  for i := 1 to Length(x) do
    if x[i] in ['!'..'~'] then {// ordinal values 33..126} begin
      Result := True;
      break;
    end;
end;
...
begin
  ...
  while (i < PieceNum) and (Next <> nil) do begin
    Inc(i);
    Strt := Next + 1;
    Next := StrScan(Strt, Delim);
  end;
...
begin
  ...
  try
    for i := 0 to AList.Count - 1 do begin
      if Length(AList[i]) > MaxLength then begin
        x := AList[i];
        while Length(x) > MaxLength do begin
          SpacePos := MaxLength;
...
begin
  with AListBox do begin
    ...
    RightSide := -2;
    for i := 0 to AHeader.Sections.Count - 1 do begin
...
begin
  while Length(AString) > 0 do begin
    ...
    if Pos(StrToFind, APiece) = 0 then begin
...
function DefMessageDlg(const Msg: string; DlgType: TMsgDlgType; Buttons:
  TMsgDlgButtons; HelpCtx: Longint; const aCaption: string = ''): Integer;
var
  Dlg: TForm;
  i: integer;
  btn: TButton;
...
//    try // rpk 3/9/2009
    ...
    try // rpk 2/23/2012
      ...
      // added ForceForgroundWindow to make sure buttons are accessible to keyboard.  rpk 9/17/2009
//      ForceForegroundWindow(dlg.Handle);
      dlg.BringToFront; // try BringToFront to move dlg to top of form stack
      {hwnd := dlg.handle;
      if hwnd > 0 then begin
        bres := BringWindowToTop(hwnd); // IE 5.5 related hack
        if bres then
...
begin
  ...
  with BCMA_Broker do begin
    ...
    // rpk 6/9/2009
    if piece(RequiredPatch, '*', 3) = '' then begin
      RequiredText := 'PSB';
      CheckType := 'B';
    end
    else begin
...
begin
  ...
  if (Result <> StringIn) and DisplayMsg then begin
...
function padr(s: string; n: integer): string;
{Pads a string to the right with spaces up to length n.  If the string is greater
than n, then truncates and concatenates an elipsis.}
var
  Len: integer;
begin
  Result := s;
